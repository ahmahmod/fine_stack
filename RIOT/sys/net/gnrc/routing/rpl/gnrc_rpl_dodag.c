...
static gnrc_rpl_parent_t *_gnrc_rpl_find_preferred_parent(gnrc_rpl_dodag_t *dodag)
{
    ...
    if (new_best != old_best) {
        ...
        //     dao_send_context_t dao_send_ctx = {.inst=dodag->instance, .destination=&old_best->addr, .fte=&fte,
...
// #if USE_FC
//     dio_send_context_t dio_send_ctx = {.inst = (uintptr_t)dodag->instance, .destination= (uintptr_t)NULL};
//     rpl_fc_execute_hook(&dio_send_ctx, sizeof(dio_send_ctx), FC_HOOK_RPL_SEND_DIO);
// #else
