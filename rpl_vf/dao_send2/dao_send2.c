...
static ipv6_addr_t *_get_me(gnrc_rpl_dodag_t *dodag){
    gnrc_netif_t *netif = bpf_gnrc_netif_get_by_prefix((ipv6_addr_t *) _GET_ELEMENT_POINTER(dodag, dodag_id));
    int idx;
    ...
    idx = bpf_gnrc_netif_ipv6_addr_match(netif, (ipv6_addr_t *)_GET_ELEMENT_POINTER(dodag, dodag_id));
    if (idx < 0) {
...
int32_t dao_send2(dao_send_context_t *ctx)
{
    ...
    if (local_instance) {
        if ((tmp = bpf_gnrc_pktbuf_add((uintptr_t)pkt, (ipv6_addr_t *)_GET_ELEMENT_POINTER(dodag, dodag_id), sizeof(ipv6_addr_t),
