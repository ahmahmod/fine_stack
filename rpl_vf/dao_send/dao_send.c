...
int32_t dao_send(dao_send_context_t *ctx)
{
    ...
    if (destination == NULL) {
        if (_GET_ELEMENT(dodag, parents) == NULL) {
            // const char msg[] = "RPL-DAO Send: dodag has no preferred parent.\n";
            // f12r_vm_printf(msg);
            res = ERROR;
            goto end;
        }

        destination = (ipv6_addr_t *)_GET_ELEMENT_POINTER(_GET_ELEMENT(dodag, parents), addr);
    }
    ...
    /* Max 31 neighbors */
    for (int i = 0; (bpf_gnrc_ipv6_nib_ft_iter(NULL, _GET_ELEMENT(dodag, iface), &ft_state, (uintptr_t)fte) && i<=31); i++) {
        ...
        if (bpf_ipv6_addr_is_global((ipv6_addr_t *)_GET_ELEMENT_POINTER(fte, dst)) && 
            !bpf_ipv6_addr_is_unspecified((ipv6_addr_t *)_GET_ELEMENT_POINTER(fte, next_hop))) {
            if ((pkt = _dao_target_build(pkt, (ipv6_addr_t *)_GET_ELEMENT_POINTER(fte, dst), _GET_ELEMENT(fte, dst_len))) == NULL) {
                // const char msg[] = "RPL-DAO Send - TARGET BUILD: no space left in packet buffer\n";
                // f12r_vm_printf(msg);
                res = ERROR;
                goto end;
            }
