...
{   
    ...
    int8_t ctx_res = ctx->ctx_res;

    gnrc_rpl_dodag_t *dodag = _GET_ELEMENT_POINTER(inst, dodag);
    gnrc_rpl_parent_t *dodag_parent = _GET_ELEMENT(dodag, parents);
    ...
    if (_GET_ELEMENT(inst, mop) != ((_GET_ELEMENT(dio, g_mop_prf) >> GNRC_RPL_MOP_SHIFT) & GNRC_RPL_SHIFTED_MOP_MASK)) {
        //DEBUG("RPL: invalid MOP for this instance.\n");
        const char msg[] = "RPL-DIO: invalid MOP for this instance.\n";
        f12r_vm_printf(msg);
        res = ERROR;
        goto end;
    } 
    /******************************************************** */
    uint8_t dodag_version = _GET_ELEMENT(dodag, version);
    uint8_t dio_version = _GET_ELEMENT(dio, version_number);
    trickle_t *trickle = _GET_ELEMENT_POINTER(dodag, trickle);
    
    if (GNRC_RPL_COUNTER_GREATER_THAN(dio_version, dodag_version)) {
    // if (dio_version> dodag_version) {
        if (_GET_ELEMENT(dodag, node_status) == 1) { // if ROOT
            uint8_t version_num = GNRC_RPL_COUNTER_INCREMENT(_GET_ELEMENT(dio, version_number));
            _SET_ELEMENT(dodag, version, version_num);
            bpf_trickle_reset_timer((uintptr_t)trickle);
        }
        else {
            _SET_ELEMENT(dodag, version, _GET_ELEMENT(dio, version_number));
            bpf_gnrc_rpl_local_repair((uintptr_t)dodag);
        }
    }
    else if (GNRC_RPL_COUNTER_GREATER_THAN(dodag_version, dio_version)) {
        bpf_trickle_reset_timer((uintptr_t)trickle);
        goto end;
    }
    /******************************************************** */
    if (_GET_ELEMENT(dodag, node_status) == 1) { // if ROOT
        if (bpf_byteorder_ntohs(_GET_ELEMENT(dio, rank)) != GNRC_RPL_INFINITE_RANK) { // Infinite Rank
            bpf_trickle_increment_counter((uintptr_t)trickle);
        }
        else {
            bpf_trickle_reset_timer((uintptr_t)trickle);
        }
    ...
    if (_GET_ELEMENT(parent, state) == 0) { // Unused parent
        if ((bpf_byteorder_ntohs(_GET_ELEMENT(dio, rank)) == GNRC_RPL_INFINITE_RANK)
            && (_GET_ELEMENT(dodag, my_rank) != GNRC_RPL_INFINITE_RANK)) {
            bpf_trickle_reset_timer((uintptr_t)trickle);
        }
        goto end;
    }
    ...
    /* incoming DIO is from pref. parent */
    else if ((_GET_ELEMENT(parent, state) == 1) && parent == _GET_ELEMENT(dodag, parents)) {
        if (_GET_ELEMENT(parent, dtsn) != _GET_ELEMENT(dio, dtsn)) {
            bpf_gnrc_rpl_delay_dao((uintptr_t)dodag, false);
        }
        //parent->dtsn = dio->dtsn;
        _SET_ELEMENT(parent, dtsn, _GET_ELEMENT(dio, dtsn));
        ...
        parse_res = parse_options(GNRC_RPL_ICMPV6_CODE_DIO, inst, opts, len, src, &included_opts);
        if(!parse_res) {
