                                  ...
                                  bool interested)
{
    ...
    /*Add FC-enabled UDP layer*/
    /*Add FC-enabled UDP layer*/
    #ifdef UDP_NO_THREAD
        if(pkt->type == GNRC_NETTYPE_UDP){
            fc_execute_hook(pkt, sizeof(pkt), FC_HOOK_UDP_RECV);
    }
    #else
...
static int _fill_ipv6_hdr(gnrc_netif_t *netif, gnrc_pktsnip_t *ipv6)
{
    ...
    {
        if (res != -ENOENT)
        { /* if there is no checksum we are okay */
                          ...
                          uint8_t netif_hdr_flags)
{
    ...
    /*1 for non-storing, 0 for storing*/
    #if (USE_FC && USE_RPL)
        if (gnrc_rpl_get_mode() == 1){ /* Non-storing */
            #ifndef MODULE_GNRC_SIXLOWPAN_BORDER_ROUTER_DEFAULT
            gnrc_netif_t *iface = gnrc_netif_get_by_prefix(gnrc_rpl_get_root_dodag_id());
            if(ipv6_addr_is_link_local(&ipv6_hdr->dst)){
                netif = gnrc_netif_get_by_pid(iface->pid);
            }
            #else
            if (netif->pid != 6 && ipv6_addr_is_link_local(&(ipv6_hdr->dst))){
                gnrc_netif_t *iface = gnrc_netif_get_by_prefix(gnrc_rpl_get_root_dodag_id());
                if(ipv6_addr_is_link_local(&ipv6_hdr->dst)){
                    netif = gnrc_netif_get_by_pid(iface->pid);
                }
    ...
    if (gnrc_ipv6_nib_get_next_hop_l2addr(&ipv6_hdr->dst, netif, pkt, &nce) < 0)
    {
    ...
    #ifdef  MODULE_GNRC_SIXLOWPAN_BORDER_ROUTER_DEFAULT 
        // #if USE_RPL
        if (netif->pid == 6){
            if (ipv6_addr_is_unspecified(&ipv6_hdr->src) && !ipv6_addr_is_link_local(&ipv6_hdr->dst)){
                ipv6_addr_t src;
                ipv6_addr_from_str(&src, BR_ADDR);
                DEBUG("ipv6: set packet source to %s\n",
                      ipv6_addr_to_str(addr_str, &src, sizeof(addr_str)));
                ipv6_hdr->src = src; 
            }
...
/* functions for sending in non-storing mode */
...
static void _send(gnrc_pktsnip_t *pkt, bool prep_hdr)
{
...
#if (USE_FC && USE_RPL)
    if (gnrc_rpl_get_mode() == 1){
    #ifndef MODULE_GNRC_SIXLOWPAN_BORDER_ROUTER_DEFAULT
            if (get_is_root() && !ipv6_addr_is_link_local(&(ipv6_hdr->dst)) && _pkt_from_me(ipv6_hdr)){
    ...
    #else
            if (get_is_root() && (ipv6_addr_match_prefix(&(ipv6_hdr->dst), gnrc_rpl_get_root_dodag_id()) >= 64) && _pkt_from_me(ipv6_hdr)){
                pkt = gnrc_rpl_srh_insert(pkt, ipv6_hdr);
                if (gnrc_rpl_get_root_dodag_id() == NULL)
                {
                    DEBUG("ipv6: no DODAG ID available, dropping packet\n");
...
#ifdef MODULE_GNRC_RPL_SR
static inline void _handle_packet_srh(gnrc_pktsnip_t *pkt, ipv6_hdr_t *hdr, bool is_root)
{
    pkt = gnrc_pktbuf_reverse_snips(pkt);
    if (pkt == NULL) {
        DEBUG("ipv6: unable to reverse pkt from receive order to send order; dropping it\n");
        return;
    }

    if (is_root && (ipv6_addr_match_prefix(&(hdr->dst), gnrc_rpl_get_root_dodag_id()) >= 64)) {
        pkt = gnrc_rpl_srh_insert(pkt, hdr);
        if (pkt == NULL) {
            gnrc_pktbuf_release(pkt);
            DEBUG("ipv6: unable to insert RPL SRH header.\n");
            return;
        }
    }
...
static void _receive(gnrc_pktsnip_t *pkt)
{
    ...
    { /* if packet is not for me */
        ...
        if (gnrc_rpl_get_mode() == 1){
        #ifndef MODULE_GNRC_SIXLOWPAN_BORDER_ROUTER_DEFAULT
            _handle_packet_srh(pkt, hdr, get_is_root());
            return;

        #else /* MODULE_GNRC_SIXLOWPAN_BORDER_ROUTER_DEFAULT */
            if (netif->pid != 6){
                _handle_packet_srh(pkt, hdr, get_is_root());
                return;
            }
