                                  ...
                                  gnrc_pktsnip_t *payload)
{
    ...
    csum = ipv6_hdr_inet_csum(csum, pseudo_hdr->data, gnrc_nettype_to_protnum(pseudo_hdr->next->type), len);
...
void gnrc_icmpv6_demux(gnrc_netif_t *netif, gnrc_pktsnip_t *pkt)
{
    ...
    char addr_str[128];
    DEBUG("icmpv6 - src: %s, ", ipv6_addr_to_str(addr_str, &((ipv6_hdr_t *)ipv6->data)->src, sizeof(addr_str)));
    DEBUG("dst: %s\n", ipv6_addr_to_str(addr_str, &((ipv6_hdr_t *)ipv6->data)->dst, sizeof(addr_str)));
...
//     //     if ((netif->pid != 5) && (pkt != icmpv6) && _calc_csum(icmpv6, ipv6, pkt)) {
