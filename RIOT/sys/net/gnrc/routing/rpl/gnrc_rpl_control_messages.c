                   ...
                   ipv6_addr_t *dodag_id)
{
    ...
    // static char addr_str[IPV6_ADDR_MAX_STR_LEN];
    // printf("sent: src: %s, ", ipv6_addr_to_str(addr_str, src, sizeof(addr_str)));
    // printf("dst: %s.\n",
                ...
                *included_opts |= ((uint32_t) 1) << GNRC_RPL_OPT_TRANSIT;
                ...
                if (first_target == NULL) {
                    ...
                    if (!ipv6_addr_equal(&(first_target->target), &(transit->parent))){
                        ...
                        gnrc_sr_add_new_dst(&(first_target->target), sizeof(ipv6_addr_t), &(transit->parent),
                       ...
                       ipv6_addr_t *dst, uint16_t len)
{
    ...
    // //Ahmad
    // eui64_t neigh_iid = {{0}};
    // //memcpy(src, ((uint8_t*) &dis->dodag_id), 8);
    // memcpy(&neigh_iid, ((uint8_t*) src) + 8, sizeof(eui64_t));
    // uint8_t neigh_l2add[sizeof(eui64_t)];
    // int res = l2util_ipv6_iid_to_addr(NETDEV_TYPE_IEEE802154, &neigh_iid, neigh_l2add);

    // if (res > 0){
    //     gnrc_ipv6_nib_nc_del(src, iface);
    //     res = gnrc_ipv6_nib_nc_set(src, iface, (uint8_t *) &neigh_l2add, res);
    //     //printf("A neigh is added, res of add: %d\n",res);
    // } else {
    //     DEBUG("A neigh is NOT  added: res=%d\n", res);
    // }
    (void) iface;
...
// #ifdef MODULE_GNRC_RPL_SR
// static void _add_neigh_route(gnrc_rpl_dio_t *dio, kernel_pid_t iface, ipv6_addr_t *src){
//         ipv6_addr_t entry;
//         memcpy(&entry, ((uint8_t*) src), sizeof(ipv6_addr_t));
//         memcpy(&entry, ((uint8_t*) &dio->dodag_id), 8);

//         gnrc_ipv6_nib_ft_del(&entry, 128);
//         gnrc_ipv6_nib_ft_add(&entry, 128, src, iface, 0);
// }
// #endif
                       ...
                       uint16_t len)
{
    (void) dst;
    // #ifdef MODULE_GNRC_RPL_SR
    //     _add_neigh_route(dio, iface, src);
    // #endif
...
//gnrc_pktsnip_t *_dao_transit_build(gnrc_pktsnip_t *pkt, uint8_t lifetime, ipv6_addr_t *parent, uint8_t mop, bool external)
gnrc_pktsnip_t *_dao_transit_build(gnrc_pktsnip_t *pkt, uint8_t lifetime, ipv6_addr_t *parent, bool external)
{
...
void gnrc_rpl_send_DAO(gnrc_rpl_instance_t *inst, ipv6_addr_t *destination, uint8_t lifetime)
{
    ...
    // printf("DAO Sent dst: %s, ", ipv6_addr_to_str(addr_str, (ipv6_addr_t *) destination, sizeof(addr_str)));
    // printf("DAO Sent src: %s\n", ipv6_addr_to_str(addr_str, (ipv6_addr_t *) me, sizeof(addr_str)));
                       ...
                       uint16_t len)
{
    (void)iface;
    (void)dst;
    // printf("DAO RECV dst: %s, ", ipv6_addr_to_str(addr_str, (ipv6_addr_t *) dst, sizeof(addr_str)));
    // printf("source: %s\n", ipv6_addr_to_str(addr_str, (ipv6_addr_t *) src, sizeof(addr_str)));
