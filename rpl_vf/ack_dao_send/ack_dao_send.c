...
uint32_t ack_dao_send(send_dao_ack_context_t *ctx)
{
    ...
    if (inst == NULL) {
        ...
        // const char msg[] = "RPL-DAO-ACK: Error - trying to send DAO-ACK without being part of a dodag.\n";
        // f12r_vm_printf(msg);
        res = ERROR;
        goto end;
    }
    ...
    if (!(pkt = bpf_gnrc_icmpv6_build((uintptr_t)NULL, ICMPV6_RPL_CTRL, GNRC_RPL_ICMPV6_CODE_DAO_ACK, size))) {
        // const char msg[] = "RPL-DAO-ACK: Send DAOACK - no space left in packet buffer\n";
        // f12r_vm_printf(msg);
        res = ERROR;
        goto end;
    }

    icmp = (icmpv6_hdr_t *)_GET_ELEMENT(pkt, data);
    dao_ack = (gnrc_rpl_dao_ack_t *)(icmp + 1);

    _SET_ELEMENT(dao_ack, instance_id, _GET_ELEMENT(inst, id));
    if (local_instance) {
        /* set the D flag to indicate that a DODAG id is present */
        _SET_ELEMENT(dao_ack, d_reserved, GNRC_RPL_DAO_ACK_D_BIT);
        f12r_memcpy((dao_ack + 1), (void *)_GET_ELEMENT_POINTER(dodag, dodag_id), sizeof(ipv6_addr_t));
    }
    else {
        _SET_ELEMENT(dao_ack, d_reserved, 0);
    }

    _SET_ELEMENT(dao_ack, dao_sequence, ctx->seq);
    _SET_ELEMENT(dao_ack, status, 0);
