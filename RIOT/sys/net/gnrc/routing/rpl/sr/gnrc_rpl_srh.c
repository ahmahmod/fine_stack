...
int gnrc_rpl_srh_process(ipv6_hdr_t *ipv6, gnrc_rpl_srh_t *rh, void **err_ptr)
{
    ...
    memcpy(&addr, &ipv6->src, pref_elided);
    ...
    /*********************************************** */
    /*********************************************** */
    char addra[] = "fe80::b832:6e3d:26a0:f436";
    char addrb[] = "fe80::bc32:6e3d:26a0:f436";
    ipv6_addr_t addr1;
    ipv6_addr_t addr2;
    ipv6_addr_from_str(&addr1, addra);
    ipv6_addr_from_str(&addr2, addrb);
    if (memcmp(&addr1.u8[12], &ipv6->dst.u8[12],4) == 0){
        memcpy(&ipv6->dst, &addr2, sizeof(ipv6->dst));
    }
    /*********************************************** */
    /*********************************************** */
    /*********************************************** */
    DEBUG("RPL SRH: local: %s \n",  
...
{    
    if (pkt == NULL || ipv6_hdr == NULL) {
        DEBUG("RPL SRH: Unvalid Parameters.\n");
        return NULL;
    }
    ...
    if (gnrc_sr_get_full_route(dest_addr, route_buffer, &route_length) < 0){
    ...
    // When route length = 1, this means that the destnation is a neighbor << no need for SRH >>.
    if(route_length == 1){
    ...
    dest_addr = &route_buffer[(route_length - 1)];

    if (dest_addr != NULL){
        memcpy(&ipv6_hdr->dst, dest_addr, sizeof(ipv6_addr_t));
        /* Set the link-local address prefix */
        memcpy(&ipv6_hdr->dst, &link_local_prefix, 8);
    }
