                                   ...
                                   size_t *uncomp_hdr_len)
{
    ...
    if ((udp_nhc & NHC_UDP_C_ELIDED) != 0) {
        printf("6lo iphc nhc: unsupported elided checksum\n");
        return 0;
    }
    else {
...
static inline bool _compressible_nh(uint8_t nh)
{
    switch (nh) {
        ...
        /* Compression of RH and UDP may cause some issues for SRH in non-storing mode */
                                         ...
                                         uint8_t *nh)
{
    ...
    /*printf("ext type: %d - 6low: hdr size: %d - ext_t size: %d - ext_len: %d\n", 
            hdr->type, hdr->size, sizeof(ipv6_ext_t), ext_len);*/
...
#endif /* !defined(MODULE_GNRC_RPL_SR) */
