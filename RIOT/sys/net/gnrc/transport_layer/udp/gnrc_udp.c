...
#define EXECUTION_TIME_DEBUG 0
#if EXECUTION_TIME_DEBUG
#include "ztimer.h"
#endif

#define ENABLE_CSUM 0
...
static void _receive(gnrc_pktsnip_t *pkt, bool enable_csum)
{   
    /*Start recording the UDP reception time*/
    #if EXECUTION_TIME_DEBUG
    ...
    if (enable_csum && (_calc_csum(udp, ipv6, pkt) != 0xFFFF)) {
    ...
    /*print the reception time in case of successful handling*/
    #if EXECUTION_TIME_DEBUG
        uint32_t end = ztimer_now(ZTIMER_USEC);
        uint32_t duration = (uint32_t)(end-begin);
        #ifdef BOARD_NATIVE
            printf("Recv Execution Duration (GNRC UDP): %d Us\n", duration);
        #else
            printf("Recv Execution Duration (GNRC UDP): %ld Us\n", duration);
        #endif
    #endif
}
...
static void _send(gnrc_pktsnip_t *pkt)
{
    ...
    /*print the reception time in case of successful handling*/
        ...
        uint32_t duration = (uint32_t)(end-begin);
        #ifdef BOARD_NATIVE
            printf("Send Execution Duration (GNRC UDP): %d Us\n", duration);
        #else
            printf("Send Execution Duration (GNRC UDP): %ld Us\n", duration);
        #endif
    #endif
}
    ...
    while (1) {
        ...
        switch (msg.type) {
            case GNRC_NETAPI_MSG_TYPE_RCV:
                ...
                _receive(msg.content.ptr, ENABLE_CSUM);
                break;
