...
enum {
    SUIT_STATUS_IDLE,
    SUIT_STATUS_DOWNLOADING,
    SUIT_STATUS_DOWNLOADED,
    SUIT_STATUS_FAILED
} suit_download_status = SUIT_STATUS_IDLE;
...
static void _suit_handle_url(const char *url, coap_blksize_t blksize)
{
    ...
    if (size >= 0) {
        ...
        suit_download_status = SUIT_STATUS_DOWNLOADING;
        ...
        if ((res = suit_parse(&manifest, _manifest_buf, size)) != SUIT_OK) {
            ...
            suit_download_status = SUIT_STATUS_FAILED;
            return;
        }
        suit_download_status = SUIT_STATUS_DOWNLOADED;
        #if ENABLE_DEBUG
            end = ztimer_now(ZTIMER_MSEC);
            if (end>start){
                LOG_INFO("CoAP Update Time = %"PRIu32" msec\n", (end - start));
            }
        #endif
                                ...
                                void *context)
{
    ...
    return coap_reply_simple(pkt, COAP_CODE_205, buf, len,
                             0, (uint8_t *)status_str, strlen(status_str));
}
...
static ssize_t _status_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len,
    void *context)
{
    (void)context;
    const char *status_str;

    switch (suit_download_status) {
        case SUIT_STATUS_IDLE:
            status_str = "idle"; break;
        case SUIT_STATUS_DOWNLOADING:
            status_str = "downloading"; break;
        case SUIT_STATUS_DOWNLOADED:
            status_str = "downloaded"; break;
        case SUIT_STATUS_FAILED:
...
#endif
    { "/suit/status", COAP_METHOD_GET, _status_handler, NULL },
