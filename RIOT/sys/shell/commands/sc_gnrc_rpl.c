...
#if USE_FC
void _rpl_trickle_send_dio(void *args)
{
...
#endif
#if USE_FC
int _fc_root_config(char *arg1, char *arg2){
    ipv6_addr_t dodag_id;
    ...
    kernel_pid_t iface_pid = atoi(arg2);
    gnrc_netif_t *netif = gnrc_netif_get_by_pid(iface_pid);
    if (netif == NULL) {
        puts("unknown interface specified");
        return 1;
    }
    ...
    root_init_context_t root_init_ctx = {.netif=netif, .dodag_id=&dodag_id,
                        .rpl_trickle_send_dio_func_ptr= &_rpl_trickle_send_dio, .netif_pid=iface_pid};

    rpl_fc_execute_hook(&root_init_ctx, sizeof(root_init_ctx), FC_HOOK_RPL_ROOT);

    printf("successfully root configured using FC\n");
    return 0;
}
#endif
...
int _gnrc_rpl(int argc, char **argv)
{
    ...
    #if USE_FC
    else if ((argc == 4) && strcmp(argv[1], "fc_root") == 0) {
        return _fc_root_config(argv[2], argv[3]);
    }
    #endif
    ...
    #endif
    #if USE_FC
    puts("* fc_root <dodag_id> <iface_pid>\t\t\t\t - Inint a root using FC.");
    #endif
