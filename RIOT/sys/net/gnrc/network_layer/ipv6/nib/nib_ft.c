...
/* Enable debug on gnrc_rpl as well */
// #define NUM_FT_ENTRIES (16) /**< Number of entries in the forwarding table */
...
static void _check_ft_entries_count(void)
{
    gnrc_ipv6_nib_ft_t fte;
    void *state = NULL;
    int count = 0;
    ...
    // if ((count == (NUM_FT_ENTRIES + 2) ) || (count == 3)) {
        // if (!printed) {
            uint32_t now = ztimer_now(ZTIMER_MSEC);
            printf("Time when FT reached  %d entries: %" PRIu32 " ms\n", count-2, now);
        // }
        // if (count == (NUM_FT_ENTRIES+2)) {
        //     printed = true;
        // }
    // }
}
                         ...
                         uint16_t ltime)
{
    ...
    if ((iface == 0) || (iface > 7) || ((is_default_route) && (next_hop == NULL))) {
        return -EINVAL;
    }
    _nib_acquire();
    if (is_default_route) {
