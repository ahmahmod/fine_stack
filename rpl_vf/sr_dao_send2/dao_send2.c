...
static gnrc_pktsnip_t *_dao_target_build(gnrc_pktsnip_t *pkt, ipv6_addr_t *addr, uint8_t prefix_length)
{
    ...
    // target->length = sizeof(target->flags + sizeof(target->prefix_length) + sizeof(ipv6_addr_t));
    // target->flags = 0;
    // target->prefix_length = prefix_length;
    // f12r_memcpy(&target->target, addr, sizeof(ipv6_addr_t));
    ...
    f12r_memcpy(_GET_ELEMENT_POINTER(target, target), addr, sizeof(ipv6_addr_t));
...
static ipv6_addr_t *_get_me(gnrc_rpl_dodag_t *dodag){
    gnrc_netif_t *netif = bpf_gnrc_netif_get_by_prefix(&dodag->dodag_id);
    int idx;

    if (netif == NULL) {
        // const char msg[] = "RPL-DAO Send:no address configured.\n";
        // f12r_vm_printf(msg);
        return NULL;
    }
    idx = bpf_gnrc_netif_ipv6_addr_match(netif, &dodag->dodag_id);
    if (idx < 0) {
        // const char msg[] = "RPL-DAO Send: no address matching DODAG ID found\n";
        // f12r_vm_printf(msg);
        return NULL;
    }
    return bpf_gnrc_netif_get_ipv6_addr_by_idx(netif, idx);
}
...
#define CONFIG_GNRC_RPL_DAO_ACK_DELAY      (3000U)


int32_t dao_send2(dao_send_context_t *ctx)
{
    ...
    gnrc_rpl_dao_t *dao;
    uint8_t dao_seq = _GET_ELEMENT(dodag, dao_seq);

    if (pkt == NULL) {
        goto end;
    }

    ipv6_addr_t *me = _get_me(dodag);
    /* Send the "me" Target option*/
    if (me){
        if ((pkt = _dao_target_build(pkt, me, 128)) == NULL) {
            // const char msg[] = "RPL-DAO Send: no space left in packet buffer\n";
            // f12r_vm_printf(msg);
            res = ERROR;
            goto end;
    ...
    bool local_instance = (_GET_ELEMENT(inst, id) & GNRC_RPL_INSTANCE_ID_MSB) ? true : false;
    if (local_instance) {
        if ((tmp = bpf_gnrc_pktbuf_add((uintptr_t)pkt, (void *)_GET_ELEMENT_POINTER(dodag, dodag_id), sizeof(ipv6_addr_t),
    ...
    if ((tmp = bpf_gnrc_pktbuf_add((uintptr_t)pkt, NULL, sizeof(gnrc_rpl_dao_t), GNRC_NETTYPE_UNDEF)) == NULL) {
        // const char msg[] = "RPL-DAO Send: no space left in packet buffer\n";
        // f12r_vm_printf(msg);
        bpf_gnrc_pktbuf_release(pkt);
        res = ERROR;
    ...
    pkt = tmp;
    dao = _GET_ELEMENT(pkt, data);
    // dao->instance_id = inst->id;
    _SET_ELEMENT(dao, instance_id, _GET_ELEMENT(inst, id));
    if (local_instance) {
        /* set the D flag to indicate that a DODAG id is present */
        _SET_ELEMENT(dao, k_d_flags,GNRC_RPL_DAO_D_BIT);
    }
    else {
        _SET_ELEMENT(dao, k_d_flags, 0);
    }

    /* set the K flag to indicate that ACKs are required */
    _SET_ELEMENT(dao, k_d_flags, _GET_ELEMENT(dao, k_d_flags) | GNRC_RPL_DAO_K_BIT);
    _SET_ELEMENT(dao, dao_sequence, dao_seq);
    _SET_ELEMENT(dao, reserved, 0);

    if ((tmp = bpf_gnrc_icmpv6_build((uintptr_t)pkt, ICMPV6_RPL_CTRL, GNRC_RPL_ICMPV6_CODE_DAO, sizeof(icmpv6_hdr_t))) == NULL) {
        // const char msg[] = "RPL-DAO Send: no space left in packet buffer\n";
        // f12r_vm_printf(msg);
        bpf_gnrc_pktbuf_release(pkt);
        res = ERROR;
    ...
    bpf_gnrc_rpl_send((uintptr_t)pkt, _GET_ELEMENT(dodag, iface), (uintptr_t)me,
                    (uintptr_t)destination, (uintptr_t)_GET_ELEMENT_POINTER(dodag, dodag_id));

    _SET_ELEMENT(dodag, dao_seq, GNRC_RPL_COUNTER_INCREMENT(dao_seq));

    // evtimer_event_t *dao_event = (evtimer_event_t *) _GET_ELEMENT_POINTER(dodag, dao_event);
    // bpf_evtimer_add_del(gnrc_rpl_evtimer, dao_event, 1); //1: delete
    // _SET_ELEMENT(dao_event, offset, CONFIG_GNRC_RPL_DAO_ACK_DELAY);
    // bpf_evtimer_add_msg(gnrc_rpl_evtimer, (void *)_GET_ELEMENT_POINTER(dodag, dao_event), gnrc_rpl_pid);
