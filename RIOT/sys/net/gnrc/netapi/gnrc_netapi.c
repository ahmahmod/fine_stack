...
/*int gnrc_netapi_dispatch_send(gnrc_nettype_t type, uint32_t demux_ctx, gnrc_pktsnip_t *pkt)
{
    if (type == GNRC_NETTYPE_UDP && f12r_check_hook(FC_HOOK_UDP_SEND)){
        printf("SEND\n");
        udp_fc_send(pkt);
        return 0;
    }
...
/*int gnrc_netapi_dispatch_receive(gnrc_nettype_t type, uint32_t demux_ctx, gnrc_pktsnip_t *pkt)
{
   if (type == GNRC_NETTYPE_UDP && f12r_check_hook(FC_HOOK_UDP_RECV)){
        printf("RECV\n");
        udp_fc_recv(pkt);
        return 0;
    }
