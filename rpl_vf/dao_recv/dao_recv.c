    ...
    while (len_parsed < len) {
        switch (_GET_ELEMENT(opt, type)){
            ...
            case (GNRC_RPL_OPT_TARGET):
                ...
                bpf_gnrc_ipv6_nib_ft_del((ipv6_addr_t *)_GET_ELEMENT_POINTER(target, target), _GET_ELEMENT(target, prefix_length));
                bpf_gnrc_ipv6_nib_ft_add((ipv6_addr_t *)_GET_ELEMENT_POINTER(target, target), _GET_ELEMENT(target, prefix_length),
            ...
            case (GNRC_RPL_OPT_TRANSIT):
                ...
                do {
                    bpf_gnrc_ipv6_nib_ft_del((ipv6_addr_t *)_GET_ELEMENT_POINTER(first_target, target), _GET_ELEMENT(first_target, prefix_length));
                    bpf_gnrc_ipv6_nib_ft_add((ipv6_addr_t *)_GET_ELEMENT_POINTER(first_target, target), _GET_ELEMENT(first_target, prefix_length),
                                            src, _GET_ELEMENT(dodag, iface),
...
int32_t dao_recv(dao_recv_context_t *ctx)
{
    ...
    // parse_options_context_t *parse_options_ctx  = ctx->parse_options_ctx;
    // send_dao_ack_context_t *send_dao_ack_ctx = ctx->send_dao_ack_ctx;
    ...
    if ((_GET_ELEMENT(dao, k_d_flags) & GNRC_RPL_DAO_D_BIT)) {
        if (f12r_memcmp((void *)_GET_ELEMENT_POINTER(dodag, dodag_id), (ipv6_addr_t *)(dao + 1), sizeof(ipv6_addr_t)) != 0) {
