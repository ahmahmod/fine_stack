...
#define ENABLE_DEBUG_CONVERGENCE (1)
...
// #if USE_FC
//     send_dis_context_t send_dis_ctx = {.inst= (uintptr_t) NULL,  
//         .destination=(uintptr_t)&ipv6_addr_all_rpl_nodes, .options=(uintptr_t)NULL, .num_opts=0};
//     rpl_fc_execute_hook(&send_dis_ctx, sizeof(send_dis_ctx), FC_HOOK_RPL_SEND_DIS);
// #else
                                        ...
                                        bool gen_inst_id, bool local_inst_id)
{
    ...
    gnrc_rpl_rpble_update(dodag);
        ...
        if (inst->mop == GNRC_RPL_MOP_NON_STORING_MODE){
            ...
            set_is_root();
            DEBUG("SR Table Initialized");
            gnrc_rpl_set_root_dodag_id((ipv6_addr_t *)dodag_id);

            /* Need for FC-based RPL only */
            gnrc_rpl_set_mode(GNRC_RPL_MOP_NON_STORING_MODE);
        }
    #endif
...
static void _receive(gnrc_pktsnip_t *icmpv6)
{
    ...
    switch (icmpv6_hdr->code) {
        case GNRC_RPL_ICMPV6_CODE_DIS:
            ...
            #if USE_FC
                dis_rev_context_t dis_recv_ctx = {.dis = (gnrc_rpl_dis_t *)(icmpv6_hdr + 1), .src= (ipv6_hdr_t *)&ipv6_hdr->src,
                                    .dst= (ipv6_hdr_t *)&ipv6_hdr->dst, .iface=iface, .len = byteorder_ntohs(ipv6_hdr->len)};
                rpl_fc_execute_hook(&dis_recv_ctx, sizeof(dis_recv_ctx), FC_HOOK_RPL_RECV_DIS);
            #else
        ...
        case GNRC_RPL_ICMPV6_CODE_DIO:
            ...
            /* If a manual parent is needed, you can provide that (PARENT_ADDR) through the makefile or compilation time */
            #if MANUAL_PARENT
                ipv6_addr_t parent_addr;
                ipv6_addr_from_str(&parent_addr, PARENT_ADDR);
                if((memcmp(&ipv6_hdr->src, &parent_addr, sizeof(ipv6_addr_t)) != 0)){
                    break;
                }
                DEBUG("Manual Parent Address is provided\n");
            #endif
            #if USE_FC
                gnrc_rpl_parent_t *parent = NULL;
                gnrc_rpl_instance_t inst;
                dio_recv_context_t dio_recv_ctx = {.dio=(gnrc_rpl_dio_t *)(icmpv6_hdr + 1), .iface = iface, 
                        .src= (ipv6_hdr_t *)&ipv6_hdr->src, .dst= (ipv6_hdr_t *)&ipv6_hdr->dst,
                        .parent= &parent, .inst=&inst, .gnrc_rpl_pid= gnrc_rpl_pid, .len=byteorder_ntohs(ipv6_hdr->len),
                        .ctx_res=-1};
                
                rpl_fc_execute_hook(&dio_recv_ctx, sizeof(dio_recv_ctx), FC_HOOK_RPL_RECV_DIO);
            #else
        ...
        case GNRC_RPL_ICMPV6_CODE_DAO:
            DEBUG("RPL: DAO received\n");
                ...
                dao_recv_context_t dao_recv_context = {.dao = (gnrc_rpl_dao_t *)(icmpv6_hdr + 1), .src= (ipv6_hdr_t *)&ipv6_hdr->src, 
                                                .dst= (ipv6_hdr_t *)&ipv6_hdr->dst, .iface= iface, .len=byteorder_ntohs(ipv6_hdr->len)};
                rpl_fc_execute_hook(&dao_recv_context, sizeof(dao_recv_context), FC_HOOK_RPL_RECV_DAO);
            #else
...
static void _parent_timeout(gnrc_rpl_parent_t *parent)
{
    ...
    if ((parent->state >= GNRC_RPL_PARENT_STALE) &&
        ...
        parent->state++;
    // #if USE_FC
    //     send_dis_context_t send_dis_ctx = {.inst= (uintptr_t)parent->dodag->instance, 
    //         .destination=(uintptr_t)&parent->addr, .options=(uintptr_t)NULL, .num_opts=0};
    //     rpl_fc_execute_hook(&send_dis_ctx, sizeof(send_dis_ctx), FC_HOOK_RPL_SEND_DIS);
    // #else
    ...
    while (1) {
        ...
        switch (msg.type) {
            ...
            case GNRC_RPL_MSG_TYPE_DODAG_DAO_TX:
                ...
                //     dao_send_context_t dao_send_ctx = {.inst=instance, .destination=NULL, .fte=&fte,
                //                                         .pkt=NULL, .gnrc_rpl_evtimer = &gnrc_rpl_evtimer, .gnrc_pid = gnrc_rpl_pid,
                //                                         .lifetime=(&instance->dodag)->default_lifetime};
                //     rpl_fc_execute_hook(&dao_send_ctx, sizeof(dao_send_ctx), FC_HOOK_RPL_SEND_DAO);
                // #else
...
void _dao_handle_send(gnrc_rpl_dodag_t *dodag)
{
    ...
    if ((dodag->dao_ack_received == false) &&
        ...
        // #if USE_FC
        //     gnrc_ipv6_nib_ft_t fte;
        //     dao_send_context_t dao_send_ctx = {.inst=dodag->instance, .destination=NULL, .fte=&fte,
        //                                         .pkt=NULL, .gnrc_rpl_evtimer = &gnrc_rpl_evtimer, .gnrc_pid = gnrc_rpl_pid,
        //                                         .lifetime=dodag->default_lifetime};
        //     rpl_fc_execute_hook(&dao_send_ctx, sizeof(dao_send_ctx), FC_HOOK_RPL_SEND_DAO);
        //     // return;
        // #else
...
#if USE_FC
void fc_rpl_trickle_send_dio(void *args)
{
...
// #if USE_FC
//     dio_send_context_t dio_send_ctx = {.inst = (uintptr_t)inst, .destination= (uintptr_t)&ipv6_addr_all_rpl_nodes};
//     rpl_fc_execute_hook(&dio_send_ctx, sizeof(dio_send_ctx), FC_HOOK_RPL_SEND_DIO);
// #else
    gnrc_rpl_send_DIO(inst, (ipv6_addr_t *) &ipv6_addr_all_rpl_nodes);
// #endif

    // context_dio_send_t dio_send_ctx = {.inst = (uintptr_t)inst, .destination= (uintptr_t)&ipv6_addr_all_rpl_nodes};
    // int64_t res = 0;
    // f12r_hook_execute(FC_HOOK_RPL_SEND_DIO, &dio_send_ctx, sizeof(dio_send_ctx), &res);
}
#endif
...
void gnrc_rpl_configure_root(gnrc_netif_t *netif, const ipv6_addr_t *dodag_id)
{
    ...
    #if USE_FC    
        DEBUG("Configure Root with FCs...\n");    
        root_init_context_t root_init_ctx = {.netif=netif, .dodag_id=(void *)dodag_id,
                            .rpl_trickle_send_dio_func_ptr= &fc_rpl_trickle_send_dio, .netif_pid=netif->pid};
        rpl_fc_execute_hook(&root_init_ctx, sizeof(root_init_ctx), FC_HOOK_RPL_ROOT);
    #else
        DEBUG("Configure Root...\n"); 
        gnrc_rpl_root_init(CONFIG_GNRC_RPL_DEFAULT_INSTANCE, dodag_id, false, false);
    #endif

    if (ENABLE_DEBUG_CONVERGENCE){
        uint32_t start_time = ztimer_now(ZTIMER_MSEC);
        printf("[RESULT] root initialized at: %" PRIu32 "\n", start_time);
    }
}
