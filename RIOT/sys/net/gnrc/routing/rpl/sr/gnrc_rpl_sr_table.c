...
#define ENABLE_DEBUG_CONVERGENCE (1) // Enable convergence debugging
#if ENABLE_DEBUG_CONVERGENCE

#include "ztimer.h"
#define NUMBER_OF_NODES_STORING (5) // Number of nodes expected to store routes
static int ft_entry_count = 0;
// static bool convergence_reported = false;  // NEW FLAG
#endif
...
void gnrc_sr_initialize_table(ipv6_addr_t *root, kernel_pid_t iface) {
    ...
    if (ENABLE_DEBUG_CONVERGENCE){
        uint32_t end_time = ztimer_now(ZTIMER_MSEC);
        printf("[RESULT] SRH Table initialized at:  %" PRIu32 " msec\n", end_time);
    }
    
    return;
}

void gnrc_sr_deinitialize_table(void) {
    fib_deinit(&table);
    DEBUG("FIB Table Deinitialized\n");
    return;
}
...
int gnrc_sr_add_new_dst(ipv6_addr_t *child, size_t prefix, ipv6_addr_t *parent, kernel_pid_t sr_iface_id, 
    ...
    if (ENABLE_DEBUG_CONVERGENCE){
        ft_entry_count++;  // Increase counter
        // if ((ft_entry_count == NUMBER_OF_NODES_STORING) && !convergence_reported) {
            uint32_t end_time = ztimer_now(ZTIMER_MSEC);
            // printf("[RESULT] Convergence achieved: %d entries in %lu ms\n",
            //        ft_entry_count, (end_time - start_time));
            printf("[RESULT] Convergence achieved: %d entries at  %" PRIu32 "\n", ft_entry_count, end_time);
            // convergence_reported = true;  // Prevent future prints
        // }
    }
    return 0;
}
...
int gnrc_sr_delete_route(ipv6_addr_t *dst_node, size_t dst_size) {
    ...
    int res = fib_remove_entry(&table,  (uint8_t *) dst_node, dst_size);
    // DEBUG("Route to destination deleted successfully.\n"); 

    if (res == 0) {
        DEBUG("Route to destination deleted successfully.\n"); 
        if (ENABLE_DEBUG_CONVERGENCE){
            ft_entry_count--;  // Only decrease if actually removed
        }
    }
    ...
    *route_length = route_index;
    ...
    if(ENABLE_DEBUG){
        DEBUG("Route reconstructed: ");
            for (size_t i = 0; i < *route_length; ++i) {
                DEBUG("%s", ipv6_addr_to_str(addr_str, (ipv6_addr_t *)&route_buffer[i], sizeof(addr_str)));
                if (i < *route_length - 1) {
                    DEBUG(" -> ");
                }
            }
