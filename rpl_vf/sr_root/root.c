...
int32_t root(root_init_context_t *ctx)
{
    ...
    uint8_t dio_opts = 0;
    _SET_ELEMENT(dodag, dio_opts, GNRC_RPL_REQ_DIO_OPT_DODAG_CONF);
    ...
    bpf_gnrc_sr_initialize_table((ipv6_addr_t *)_GET_ELEMENT_POINTER(dodag, dodag_id), _GET_ELEMENT(dodag, iface));
    bpf_gnrc_rpl_is_root(1); // 0: get_is_set() - 1: set_is_set()
    bpf_gnrc_rpl_root_dodag_id(1, dodag_id);
    bpf_gnrc_rpl_mode(1,1); /*1: set (0 :get), 1: for non-storing (0 for Storing)*/
