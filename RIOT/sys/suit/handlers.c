                                   ...
                                   size_t handlers_len)
{
    ...
    if ((nanocbor_enter_array(it, &container) < 0) &&
        ...
        LOG_INFO("Neither array nor map: %d\n", nanocbor_get_type(it));
        return SUIT_ERR_INVALID_MANIFEST;
    }
    ...
    while (!nanocbor_at_end(&container)) {
        ...
        if (nanocbor_get_int32(&container, &key) < 0) {
            LOG_INFO("No key found: %d\n", nanocbor_get_type(&container));
            return SUIT_ERR_INVALID_MANIFEST;
        }
        nanocbor_value_t value = container;
        LOG_INFO("Executing handler with key %" PRIi32 "\n", key);
        #if ENABLE_TIME_DEBUG
            local_start = ztimer_now(ZTIMER_MSEC);
        #endif
        ...
        #if ENABLE_TIME_DEBUG
            local_end = ztimer_now(ZTIMER_MSEC);
            if (local_end>local_start){
                LOG_INFO("handler key: %" PRIi32 " - execution time = %"PRIu32" msec\n", key, (local_end - local_start));
            }
        #endif
    }
    nanocbor_leave_container(it, &container);
    LOG_INFO("Leaving sequence handler\n");
